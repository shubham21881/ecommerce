  ...
  return (
    <div  className='bg-[#BFACE2] py-3 fixed w-full'>
        <nav className='flex justify-around items-center '>
          <span className='sm:hidden'><GiHamburgerMenu /></span>
            <div className='cursor-pointer'><h1 className="text-2xl font-bold text-white">ShopEasy</h1></div>
            ...
            <span className='sm:hidden'><NavLink to={'/CartPage'}  > <li className=' relative cursor-pointer list-none   px-4'><FaCartShopping />{ Cartitem.length?<span className='  absolute top-[-23px] left-[30] w-[20px] rounded-2xl text-white bg-red-600 text-center '>{Cartitem.length}</span>:<span className=' hidden absolute top-[-11px]  rounded-2xl text-white bg-red-600 text-center '>{Cartitem.length}</span>}</li></NavLink></span>
        </nav>
    </div>
  )
}
